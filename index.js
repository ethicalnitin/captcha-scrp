...
async function fetchCaptcha(url, cookies, referer, userAgent) {
    try {
        ...
        console.error(`Error fetching captcha from ${url}:`, error.message);
        // You might want to log the full error in a real application
...
// --- Start the server ---
app.listen(port, () => {
    console.log(`Captcha proxy backend listening at http://localhost:${port}`);
});
